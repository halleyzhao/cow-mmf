...
LOCAL_INCLUDES := $(MM_INCLUDE) \
                  $(MULTIMEDIA_BASE)/wifidisplay/include  \
                  ../src

LOCAL_SHARED_LIBRARIES := mmbase cowbase cow-avhelper
## LOCAL_LDFLAGS:= -I/usr/local/include -L/usr/local/lib -lavformat -lavcodec -lavutil
LOCAL_CPPFLAGS:=`pkg-config --cflags libavformat libavcodec libavutil`
LOCAL_LDFLAGS:=`pkg-config --libs libavformat libavcodec libavutil`

LOCAL_MODULE := libRtpMuxer.so
LOCAL_INSTALL_PATH := $(INST_LIB_PATH)/cow
SRC_PATH := ../src/components

LOCAL_SRC_FILES := $(SRC_PATH)/rtp_muxer.cc
